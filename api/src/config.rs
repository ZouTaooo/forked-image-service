...
mod tests {
    ...
    fn test_backend_config_valid() {
        ...
        cfg.backend_type = "localdisk".to_string();
        assert!(!cfg.validate());
        cfg.localdisk = Some(LocalDiskConfig {device_path: "".to_string(), disable_gpt: true});
        assert!(!cfg.validate());
        
        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "localfs".to_string();
        assert!(!cfg.validate());
        
        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "oss".to_string();
        assert!(!cfg.validate());
        
        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "s3".to_string();
        assert!(!cfg.validate());

        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "register".to_string();
        assert!(!cfg.validate());

        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "http-proxy".to_string();
        assert!(!cfg.validate());
        
        let mut cfg:BackendConfigV2 = BackendConfigV2::default();
        cfg.backend_type = "foobar".to_string();
        assert!(!cfg.validate());   
    }
    ...
    fn test_cache_config_valid() {
        ...
        cfg.cache_type = "blobcache".to_string();
        assert!(!cfg.validate());

        cfg.cache_type = "fscache".to_string();
        assert!(!cfg.validate());

        cfg.cache_type = "dummycache".to_string();
        assert!(cfg.validate());

        cfg.cache_type = "foobar".to_string();
        assert!(!cfg.validate());
    }
    ...
    fn test_blob_cache_entry_v2_from_file() {
        ...
        if fs::write("test_blob_cache_entry_v2_from_file.cfg", content).is_ok() {
            let res = BlobCacheEntryConfigV2::from_file("test_blob_cache_entry_v2_from_file.cfg").is_ok();
