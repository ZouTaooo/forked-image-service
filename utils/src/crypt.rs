...
mod tests {
    ...
    fn test_attribute() {
        ...
        assert!(Algorithm::try_from(Algorithm::None).is_ok());
        assert!(Algorithm::try_from(Algorithm::Aes128Xts).is_ok());
        assert!(Algorithm::try_from(Algorithm::Aes256Xts).is_ok());
        assert!(Algorithm::try_from(Algorithm::Aes256Gcm).is_ok());
        assert!(Algorithm::try_from(4 as u64).is_err());

        print!("{:?},{:?},{:?},{:?}\n", none, aes128xts, aes256xts, aes256gcm);
    }
    ...
    fn test_crypt_with_context() {
        ...
        assert!(CipherContext::new(error_key.to_vec(), iv.to_vec(), true, Algorithm::Aes128Xts).is_err());
        // create wtih symmetry key
        assert!(CipherContext::new(symmetry_key.to_vec(), iv.to_vec(), true, Algorithm::Aes128Xts).is_err());

        let ctx = CipherContext::new(key.to_vec(), iv.to_vec(), false, Algorithm::Aes128Xts).unwrap();
        ...
        let plain_text_no_change = decrypt_with_context(&bind, &obj, &Some(ctx.clone()), false).unwrap();
    ...
    fn test_gen_key(convergent_encryption: bool) {
        ...
        let  iv = [0u8; 16];
        let data = b"11111111111111111";
        let ctx = CipherContext::new(key.to_vec(), iv.to_vec(), convergent_encryption, Algorithm::Aes128Xts).unwrap();
        let obj = Arc::new(Algorithm::Aes128Xts.new_cipher().unwrap());
        let (gen_key, gen_iv) = ctx.generate_cipher_meta(&key);
        let ciphertext3 = obj.encrypt(gen_key, Some(&gen_iv), data).unwrap();
        let plaintext3 = obj.decrypt(gen_key, Some(&gen_iv), &ciphertext3).unwrap();
        assert_eq!(&plaintext3, data);
    }
    #[test]
