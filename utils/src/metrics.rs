...
mod tests {
    ...
    fn test_file_stats_update() {
        ...
        if f.access_pattern_enabled() {
            assert_eq!(f.access_patterns.read().unwrap().get(&node1).unwrap().nr_read.count(), 2);
        }
        if f.files_enabled() {
           assert_eq!(f.file_counters.read().unwrap().get(&node1).unwrap().fop_hits[StatsFop::Read as usize].count(), 2);
        }
    }
    ...
    fn test_latecny() {
        let f = FsIoStats::default();
        let Some(s) = f.latency_start() else { return; };
        let d = Duration::new(2, 0);
        f.latency_end(&s.checked_sub(d), StatsFop::Read);
        
        assert_eq!(f.read_latency_dist[latency_micros_range_index(2 * 1000 * 1000)].count(), 1);
        assert_eq!(f.fop_cumulative_latency_total[StatsFop::Read as usize].count(), saturating_duration_micros(&d));
    }
    ...
    fn test_blob_cache_metric() {
        let m1 = BlobcacheMetrics::new("id", "path");
        {
            let metrics = BLOBCACHE_METRICS.read().unwrap();
        ...
        assert_eq!(saturating_duration_millis(&Duration::from_millis(1234)), 1234);
        assert_eq!(saturating_duration_micros(&Duration::from_millis(888)), 888_000);
        assert_eq!(saturating_duration_micros(&Duration::from_millis(1888)), 1888_000);
        ...
        assert_eq!(m1.prefetch_end_time_secs.count(), now.duration_since(SystemTime::UNIX_EPOCH).expect("No error").as_secs());
